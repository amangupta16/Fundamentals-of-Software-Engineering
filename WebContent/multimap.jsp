...
<body>
	...
	<h2>FLight Maps</h2>
		...
		<%
			...
			String fend = request.getParameter("fdestination");
			float startLat=0;
			float startLon=0;
			float endLat=0;
			float endLon=0;
			...
			try 
			{
				...
				while (rs.next()) 
				{
					...
					if (rs.getString("airport").equals(fend))
					{
						fendLat = rs.getFloat("latitude");
						fendLon = rs.getFloat("longitude");
					}
				}
		...
		<script>
		      function initMap() {
		    	...
		    	var fendLocation = {lat: <%=fendLat%>, lng: -<%=fendLon%>};
		        ...
		        var fendMarker = new google.maps.Marker({
		            position: fendLocation,
		            map: map,
		            title: 'Click to zoom'
		          });
		        ...
		        var line = new google.maps.Polyline({
		            path: [new google.maps.LatLng(<%=endLat%>, -<%=endLon%>), new google.maps.LatLng(<%=fendLat%>, -<%=fendLon%>)],
		            strokeColor: "#FF0000",
		            strokeOpacity: 1.0,
		            strokeWeight: 10,
		            geodesic: true,
		        ...
		        endMarker.addListener('mouseover', function() {
		        	var infowindow = new google.maps.InfoWindow({content:"Layover: <%=end%>"});
		        	infowindow.open(map,endMarker);
		        });
		        
		        fendMarker.addListener('click', function() {
		        	map.setZoom(8);
		        	map.setCenter(fendMarker.getPosition());
		        });
		        
		        fendMarker.addListener('mouseover', function() {
		        	var infowindow = new google.maps.InfoWindow({content:"Arrival: <%=fend%>"});
		        	infowindow.open(map,fendMarker);
		        });
